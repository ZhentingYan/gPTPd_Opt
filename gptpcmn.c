...
u64 gptp_getCurrMilliSecTS(void) {
    ...
    currTickTS = ((ts.tv_sec * 1000) + (int)(ts.tv_nsec * 0.000001));
    return currTickTS;
}

void gptp_startTimer(struct gPTPd *gPTPd, u32 timerId, u32 timeInterval,
                     u32 timerEvt) {
...
void gptp_copyTSFromBuf(struct timespec *ts, u8 *src) {
    ...
    gPTP_logMsg(GPTP_LOG_DEBUG,
                "fb: src  %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x \n",
    ...
    if (sizeof(time_t) == 8) {
        ...
        ts->tv_nsec = srcTS->ns;
        gPTP_logMsg(GPTP_LOG_DEBUG,
                    "fb: dest %02x %02x %02x %02x %02x %02x %02x %02x %02x "
                    "%02x %02x %02x \n",
                    dest[0], dest[1], dest[2], dest[3], dest[4], dest[5],
                    dest[6], dest[7], dest[8], dest[9], dest[10], dest[11]);
    } else {
        ts->tv_sec = srcTS->s.lsb;
        ts->tv_nsec = srcTS->ns;
        gPTP_logMsg(GPTP_LOG_DEBUG,
                    "fb: dest %02x %02x %02x %02x %02x %02x %02x %02x \n",
                    dest[0], dest[1], dest[2], dest[3], dest[4], dest[5],
                    dest[6], dest[7]);
    }
}
...
void gptp_copyTSToBuf(struct timespec *ts, u8 *dest) {
    ...
    if (sizeof(time_t) == 8) {
        gPTP_logMsg(GPTP_LOG_DEBUG,
                    "tb: src  %02x %02x %02x %02x %02x %02x %02x %02x %02x "
                    "%02x %02x %02x \n",
        ...
        gPTP_logMsg(GPTP_LOG_DEBUG,
                    "tb: src %02x %02x %02x %02x %02x %02x %02x %02x \n",
                    src[0], src[1], src[2], src[3], src[4], src[5], src[6],
                    src[7]);
        destTS->s.msb = 0;
    ...
    gPTP_logMsg(
        GPTP_LOG_DEBUG,
        "tb: dest  %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x \n",
...
void getTxTS(struct gPTPd *gPTPd, struct timespec *ts) {
    ...
    do {
        ...
        if (res < 1) {
            ...
            gPTP_logMsg(GPTP_LOG_ERROR,
                        "poll for tx timestamp woke up on non ERR event");
            break;
        } else {
            ...
            for (cm = CMSG_FIRSTHDR(&gPTPd->rxMsgHdr); cm != NULL;
                ...
                if (SOL_SOCKET == level && SO_TIMESTAMPING == type) {
                    if (cm->cmsg_len < sizeof(*ts) * 3) {
                        ...
                        //循环展开
                        if ((rts[0].tv_sec != 0) || (rts[0].tv_nsec != 0)) {
                            if (ts != NULL) {
                                ts->tv_sec = rts[0].tv_sec;
                                ts->tv_nsec = rts[0].tv_nsec;
                            }
                            gPTP_logMsg(GPTP_LOG_INFO,
                                        "TxTS: %d: sec: %d nsec: %d \n", 0,
                                        rts[0].tv_sec, rts[0].tv_nsec);
                        }
                        if ((rts[1].tv_sec != 0) || (rts[1].tv_nsec != 0)) {
                            if (ts != NULL) {
                                ts->tv_sec = rts[1].tv_sec;
                                ts->tv_nsec = rts[1].tv_nsec;
                            }
                            gPTP_logMsg(GPTP_LOG_INFO,
                                        "TxTS: %d: sec: %d nsec: %d \n", 1,
                                        rts[1].tv_sec, rts[1].tv_nsec);
                        }
                        if ((rts[2].tv_sec != 0) || (rts[2].tv_nsec != 0)) {
                            if (ts != NULL) {
                                ts->tv_sec = rts[2].tv_sec;
                                ts->tv_nsec = rts[2].tv_nsec;
                            }
                            gPTP_logMsg(GPTP_LOG_INFO,
                                        "TxTS: %d: sec: %d nsec: %d \n", 2,
                                        rts[2].tv_sec, rts[2].tv_nsec);
                        }
                        // for (int i = 0; i < 3; i++)
                        // 	if ((rts[i].tv_sec != 0) || (rts[i].tv_nsec !=
                        // 0))
                        // 	{
                        // 		if (ts != NULL)
                        // 		{
                        ...
                        // 		}
                        // 		gPTP_logMsg(GPTP_LOG_INFO, "TxTS: %d:
                        // sec: %d nsec: %d \n", i, rts[i].tv_sec,
                        // rts[i].tv_nsec);
                        // 	}
                    }
    ...
    for (cm = CMSG_FIRSTHDR(&gPTPd->rxMsgHdr); cm != NULL;
        ...
        if (SOL_SOCKET == level && SO_TIMESTAMPING == type) {
            if (cm->cmsg_len < sizeof(*ts) * 3) {
                ...
                // 			ts->tv_nsec = rts[i].tv_nsec;
                // 		}
                // 		gPTP_logMsg(GPTP_LOG_INFO, "RxTS: %d: sec: %d
