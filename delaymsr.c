...
#define LEN_TIMESPC sizeof(struct timespec)
...
void dmHandleEvent(struct gPTPd *gPTPd, int evtId)
{
	...
	case DM_STATE_IDLE:
		...
		case GPTP_EVT_DM_PDELAY_REQ_RPT:
			...
			memset(&gPTPd->ts[0], 0, len_timespc);
			sendDelayReq(gPTPd);
	...
	case DM_STATE_DELAY_RESP_FLWUP_WAIT:
		...
		case GPTP_EVT_DM_PDELAY_RESP_FLWUP:
			...
			gPTP_logMsg(GPTP_LOG_INFO, "@@@ t%d: %lld_%ld\n", 1, (s64)gPTPd->ts[0].tv_sec, gPTPd->ts[0].tv_nsec);
			gPTP_logMsg(GPTP_LOG_INFO, "@@@ t%d: %lld_%ld\n", 2, (s64)gPTPd->ts[1].tv_sec, gPTPd->ts[1].tv_nsec);
			gPTP_logMsg(GPTP_LOG_INFO, "@@@ t%d: %lld_%ld\n", 3, (s64)gPTPd->ts[2].tv_sec, gPTPd->ts[2].tv_nsec);
...
{
	int err = 0;
	int local_len_ethhdr = len_ethhdr;
	int local_len_gPTPHdr = len_gPTPHdr;
	int local_len_sockaddr_ll = len_sockaddr_ll;
